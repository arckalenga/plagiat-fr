...
function Attente() {
  const { user, profil, chargement, rafraichirProfil } = useAuth();
  useEffect(() => { if (user) void rafraichirProfil(); }, [user, rafraichirProfil]);
  if (chargement) return <div className="grid min-h-screen place-items-center bg-sand">Vérification de votre compte…</div>;
  if (!user) return <Navigate to="/connexion" replace />;
  if (profil?.statut === "approuve") return <Navigate to={profil.role === "administrateur" ? "/admin" : "/app"} replace />;
  return <div className="p-6 lg:p-10"><h1 className="font-display text-4xl text-forest-900">Compte en attente d’approbation</h1><p className="mt-3 text-black/50">Votre demande a bien été enregistrée. Cette page s’actualisera dès que votre accès sera approuvé.</p></div>;
}
